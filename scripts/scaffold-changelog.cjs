...
const GENERATED_FILE_NOTICE = '<!-- Generated by scripts/scaffold-changelog.cjs. Do not edit manually. -->';
const SECTION_ORDER = ['features', 'fixes', 'optimizations', 'misc'];
const SECTION_TITLES = {
  features: '### 新功能 / Features',
  fixes: '### 修复 / Fixes',
  optimizations: '### 优化 / Optimizations',
  misc: '### 杂项 / Misc',
};
...
const run = () => {
  ...
  if (args.check) {
    if (!fs.existsSync(outputPath)) {
      throw new Error(`Missing changelog snapshot: ${relativeOutputPath}`);
    }

    const existingMarkdown = fs.readFileSync(outputPath, 'utf8');
    if (existingMarkdown !== markdown) {
      throw new Error(
        `Changelog snapshot is out of date: ${relativeOutputPath}. Re-run with --force.`
      );
    }

    process.stdout.write(
      `Verified ${relativeOutputPath} matches ${resolvedRange} (${commitCount} commit${commitCount === 1 ? '' : 's'}).\n`
    );
    return;
  }
  ...
  if (fs.existsSync(outputPath) && !args.force) {
    throw new Error(`Refusing to overwrite existing changelog: ${relativeOutputPath}`);
  }
