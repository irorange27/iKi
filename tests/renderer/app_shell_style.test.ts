...
describe('renderer app shell styles', () => {
  ...
  it('lets the chat column and tool cards shrink cleanly when the sidebar narrows content space', () => {
    ...
    expect(chatSource).toMatch(/\.chat-main-area\s*\{[\s\S]*overscroll-behavior:\s*contain;/i);
    expect(chatSource).toMatch(/\.chat-main-area\s*\{[\s\S]*scrollbar-gutter:\s*stable both-edges;/i);
    expect(chatSource).toMatch(
      /\.chat-main-area-welcome\s*\{[\s\S]*align-items:\s*flex-start;[\s\S]*justify-content:\s*center;/i
    );
